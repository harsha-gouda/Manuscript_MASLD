...
data_transpose <- data_transpose %>% filter(filename %in% m_filter$filename)

# Remove near-zero variance features
nzv_cols <- nearZeroVar(data_transpose)
data_transpose <- data_transpose[, -nzv_cols]
...
```{r PLSDA}
filter_data <- clr_data %>% filter(filename %in% m_filter$filename)
...
perf_plsda <- mixOmics::perf(PLSDA_whole, validation = "Mfold", folds = 20, 
                              progressive = TRUE, auc = TRUE)

cv_error <- perf_plsda$error.rate$overall %>% as.data.frame()
print(cv_error$centroids.dist)
```
